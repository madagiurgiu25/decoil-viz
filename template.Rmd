...
output:
  ...
  html_document:
    ...
    keep_md: yes
params:
  # root: "/examples/ecnda1"
  # rootreal: "/examples/ecdna1"
  rootimg: "reconstruct/"
  # nodefile: "reconstruct.bed"
  # edgesfile: "reconstruct.links.txt"
  # summaryfile: "summary.txt"
  # bw: "/examples/ecdna1/coverage.bw"
  # fname: "reconstruct.test.html"
  # sample: "test"
  # maxcov: 600
  # refgenome: "/annotation/reference.fa"
  # gtffile: "/annotation/anno.gtf"
  # genesfile: "/decoil/decoil/../annotation/genes.txt"
  root: "/Users/madag/Projects/PhD/github/decoil-build/test_plot/"
  rootreal: "/Users/madag/Projects/PhD/github/decoil/examples/ecdna1/"
  # root: "/Users/madag/Projects/PhD/github/decoil-build/test/"
  nodefile: "reconstruct.bed"
  edgesfile: "reconstruct.links.txt"
  summaryfile: "summary.txt"
  bw: "/Users/madag/Projects/PhD/github/decoil-build/test_plot/coverage.bw"
  fname: "/Users/madag/Projects/PhD/github/decoil-build/test_plot/reconstruct.html"
  sample: "ecdna1"
  maxcov: 100
  refgenome: "/Users/madag/Projects/PhD/reference/GRCh38.primary_assembly.genome.fa"
  gtffile: "/Users/madag/Projects/PhD/annotation/gencode.v42.primary_assembly.basic.annotation.gtf"
  genesfile: "/Users/madag/Projects/PhD/github/decoil/annotation/genes.txt"
  # window: "/Users/madag/Projects/PhD/github/decoil-build/test_plot/window.bed"
  window: ""
  decoilversion: "1.1.2"
  filtertop: ""
  filterscore: ""
...
root <- params$root
rootimg <- params$rootimg
# the path outside the container, i.e. on host
rootreal <- params$rootreal

dir.create(file.path(root))
dir.create(file.path(paste0(rootreal,rootimg)))

nodesfile <- paste0(root,params$nodefile)
edgesfile <- paste0(root, params$edgesfile)
summfile <- paste0(root, params$summaryfile)
bwfile <- params$bw
sample <- params$sample
maxcov <- params$maxcov
htmlfile <- params$fname

decoilversion <- params$decoilversion
...
>**Date**: `r Sys.Date()` <br/>
>**Decoil version**: `r decoilversion`
...
>**Genomic regions**: `r paste0(rootreal,nodesfile)` <br/>
>**Breakpoints information**: `r paste0(rootreal,edgesfile)` <br/>
...
# gencode = gTrack(filtered_gr, name = 'Gencode', gr.labelfield = 'gene_name', grl.labelfield = 'gene_name', gr.cex.label = 2, height = 90, stack.gap = 1e+06, xaxis.unit = 5, grl.colorfield = 'yellow', sep.bg.col = 'white', path.col = "yellow", gr.srt.label = 2, col=alpha('#159957', .5))

# gencode <- track.gencode(gencode = filtered_gr, stack.gap = 1e+06, cex.label = 0.6, height = 90, cached = FALSE, build = NULL,
#                         name = 'Gencode', labels.suppress.gr=T, gene.collapse = T,
#                         genes=read.csv2(genes,header=F,"\t")$V1,
#                         bg.col = alpha("black", 1),
#                         cds.col = alpha("black", 0),
#                         utr.col = alpha("black",0),
#                         st.col = alpha("black",0),
#                         en.col = alpha("black",0))
...
#### 4. Draw circles
draw_amplicon <- function(s, bwpath, maxcov, df_edges, gr_nodes, df_summary, windowfile){
  ...
  for( circ in individual_circles){
    ...
    fcirc <- paste0(rootreal,rootimg, circ, ".pdf")
...
#### 5. Draw individuals 
out <- draw_amplicon(sample, bwfile, maxcov, df_edges, gr_nodes, df_summary, windowfile)
p1 <- out$gplots
...
# as pdf
fcirc <- paste0(rootreal, rootimg, "all_ecdna_elements.pdf")
